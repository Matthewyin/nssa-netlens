# NetLens (网络透镜)

**专业的 AI 驱动网络流量分析与可视化平台** | v0.3.0

NetLens 将 Wireshark 的深度解析能力与现代 AI 的推理能力相结合，为网络工程师、安全研究员提供了一个高性能、可视化、智能化的分析工具。

---

## 👨‍💻 作者与交流

*   **作者**: AlkaiDY
*   **邮箱**: tccio2023@gmail.com
*   **GitHub**: [https://github.com/Matthewyin/nssa-netlens](https://github.com/Matthewyin/nssa-netlens)

欢迎提交 Issue 或 PR 参与贡献！

---

## 💡 核心价值

传统的网络分析工具（如 Wireshark）功能强大但门槛极高，用户需要记忆复杂的过滤语法，并人工从海量数据中寻找线索。NetLens 旨在解决以下痛点：

1.  **智能化 (Intelligent)**: 内置 AI 分析师，用户可以通过自然语言提问（如"帮我看看有没有 SQL 注入"），AI 自动调用工具进行排查。
2.  **可视化 (Visual)**: 提供交互式 TCP 时序图、全量包头详情树、流量趋势图，将枯燥的数据转为直观的图表。
3.  **关联性 (Correlated)**: 独有的"多文件关联分析"功能，能够自动对比防火墙内外的抓包文件，精准定位丢包和时延。
4.  **高性能 (Performance)**: 底层基于 Tshark 流式解析，内存占用极低，轻松处理 GB 级大文件。

---

## 🏗️ 技术架构

NetLens 采用现代化的混合架构，兼顾了前端的灵活性与后端的性能。

```mermaid
graph TD
    User["用户"] -->|交互| UI["Electron + React 前端"]
    UI -->|IPC 通信| Main["Electron 主进程"]
    
    subgraph "AI 智能体 (Agent Runtime)"
        Main -->|"Prompt + Tools"| LLM["大模型 (OpenAI/DeepSeek)"]
        LLM -->|"Function Call"| Main
    end
    
    subgraph "分析引擎 (Analysis Engine)"
        Main -->|Spawn| Python["Python 后端"]
        Python -->|"Stream/JSON"| Tshark["Wireshark CLI (Tshark)"]
        Tshark -->|"Raw Packets"| File[("PCAP 文件")]
    end
```

### 🤖 AI 调用逻辑 (ReAct 模式)

NetLens 不仅仅是把数据发给 AI，而是实现了一个完整的 **ReAct (Reasoning + Acting)** 循环：

1.  **感知**: 用户输入自然语言问题。
2.  **思考**: AI 分析用户意图，决定需要调用哪些工具（如 `scan_security_threats`）。
3.  **行动**: Electron 拦截 AI 的指令，在本地调用 Python/Tshark 执行实际分析。**原始 PCAP 数据永远不会上传给 AI，仅上传脱敏后的统计结果 (JSON)**，最大程度保护隐私。
4.  **反馈**: 工具执行结果返回给 AI。
5.  **回答**: AI 综合数据生成最终报告。

---

## 🌟 功能特性

### 1. TCP 会话分析
*   **通讯对列表**: 展示所有 TCP 会话，包含源/目的 IP、端口、包数、流量、持续时间。
*   **交互式流视图**: 点击会话展开，以类似聊天气泡的方式展示数据包交互过程，左侧为接收、右侧为发送。
*   **协议分层树**: 点击任意数据包气泡，底部展示完整的协议栈详情（Frame → Ethernet → IP → TCP → 应用层）。

### 2. 故障诊断 (Troubleshooting)
*   **TCP 健康体检**: 自动检测重传 (Retransmission)、零窗口 (Zero Window)、乱序、Reset 等异常。
*   **异常会话聚合**: 按会话分组展示异常事件，快速定位问题连接。

### 3. 链路追踪 (Link Trace)
*   **多跳可视化**: 针对多点抓包场景，自动关联并展示请求在不同节点间的跳转路径。
*   **三栏仪表盘**: 左侧链路列表、中间可视化拓扑、右侧详情面板，支持 1:1:1 比例拖拽调节。
*   **深度钻取**: 点击拓扑中的任意 Hop，可展开查看该段链路的数据包列表；点击数据包可查看详细解码。
*   **Wireshark 联动**: 支持一键跳转 Wireshark 并定位到特定数据包。

### 4. 协议深度分析
*   **HTTP**: 提取 User-Agent, Content-Type, Cookie，支持 JSON/Text Body 预览。
*   **TLS**: 解析握手信息，展示证书链 (Issuer, Subject, Validity)。
*   **DNS**: 识别 NXDOMAIN 错误，展示完整的查询/响应树。

### 5. 安全分析
*   **端口扫描检测**: 识别单一 IP 对多端口的 SYN 探测行为。
*   **SQL 注入 / XSS 检测**: 基于 Payload 正则匹配常见攻击特征。
*   **明文凭证告警**: 检测 Basic Auth 等敏感信息。

---

## 🛠 安装与使用

### 前置要求
*   **macOS** (目前仅支持 Mac)
*   系统需安装 **Wireshark** (确保 `tshark` 命令可用)。

### 安装方法 (macOS)
1.  进入 [Releases](https://github.com/Matthewyin/nssa-netlens/releases) 页面。
2.  下载最新的 `.dmg` 安装包 (例如 `NetLens-0.2.4-arm64.dmg`)。
3.  双击 `.dmg` 文件，将 NetLens 图标拖入 Applications 文件夹。
4.  **首次运行**: 由于未签名，可能需要按住 `Control` 键点击图标，选择“打开”，然后在弹窗中再次点击“打开”。

### 常见问题 (App 已损坏 / 无法打开)
如果遇到提示“应用已损坏，无法打开”或“无法验证开发者”，请在终端 (Terminal) 执行以下命令修复：

```bash
xattr -cr /Applications/NetLens.app
```
(如果应用不在应用程序目录，请将路径替换为实际位置，如 `~/Downloads/NetLens.app`)

### 使用指南
1.  **启动**: 打开 NetLens。
2.  **配置 AI**: 点击左下角 `⚙️ 设置`，输入您的 API Key 和 Base URL (支持 OpenAI, DeepSeek, Ollama 等)。点击"验证"确保连接成功。
3.  **导入数据**: 点击左上角 `+` 选择一个或两个 `.pcap` 文件。
4.  **开始分析**:
    *   **手动模式**: 点击侧边栏的 "HTTP"、"故障诊断"、"链路追踪" 等按钮查看报表。
    *   **AI 模式**: 点击右侧 "🤖 AI 分析师"，输入 "分析一下这个包的异常"，等待 AI 给出报告。

---

## 📋 更新日志

### v0.3.0 (2026-01-31)
*   **重构**: UI 全面升级 - 引入紧凑型头部 (Compact Header)，大幅提升信息密度和屏幕利用率。
*   **新增**: 链路追踪搜索功能 - 支持按 Chain ID 和 Method 实时过滤链路。
*   **优化**: 侧边栏导航 - 采用现代化 SVG 图标，整合“关于”菜单和 GitHub 链接。
*   **修复**: 对比分析页面显示异常 - 修复了对比分析结果在切换 Tab 后未正确渲染的问题。
*   **修复**: 交互体验优化 - 修复了表格滚动、下拉菜单点击外部关闭等细节问题。

### v0.2.6 (2025-01-30)
*   **新增**: 报告导出功能 - 支持将分析结果导出为精美的 HTML 报表或原始 JSON 数据。
*   **优化**: 性能大幅提升 - 引入虚拟滚动 (Virtual Scrolling) 技术，轻松流畅展示数万条数据包。
*   **优化**: 实时进度反馈 - 界面实时显示分析阶段（如“扫描端口中”、“解析协议中”），告别假死状态。
*   **重构**: 全面清理前端内联样式，统一 UI 风格；修复后端类型提示警告，提升代码健壮性。
*   **新增**: Wireshark 路径可配置 - 在设置中自定义 Wireshark 安装路径。
*   **重构**: 开发服务器端口改为 61111，避免与其他服务冲突。
*   **重构**: main.js 通用 Python 命令执行函数，减少重复代码。
*   **重构**: analyzer.py 魔法数字常量化，提高代码可维护性。

### v0.2.5 (2025-01-30)
*   **修复**: 对比分析菜单点击无反应的问题 - 移除了错误的条件判断，现在点击"对比分析"会正常触发分析。

### v0.2.4 (2025-01-29)
*   **重构**: 链路追踪 (Link Trace) 界面全新升级，采用三栏式仪表盘布局 (1:1:1)，大幅提升大屏利用率。
*   **新增**: 数据包详情查看功能，支持在应用内直接查看 Packet Details 协议树。
*   **新增**: Wireshark 联动功能，支持从链路追踪界面一键跳转 Wireshark 并定位到特定数据包。
*   **优化**: 加载动画升级为百分比进度条，状态反馈更直观。
*   **修复**: TCP 会话列表在某些视图下显示异常的问题。

### v0.2.3 (2025-01-28)
*   **修复**: TCP 会话分析中点击数据包气泡导致黑屏的问题（PacketDetailTree 组件未导入）
*   **增强**: 完善 TCP 会话的交互式流视图功能描述

### v0.2.2
*   文档和许可证更新

### v0.2.1
*   UI/UX 修复和 AI 增强

### v0.2.0
*   新增 AI Copilot、分屏视图、多文件关联分析

---

## ⚠️ 版权声明

本项目 (**NetLens**) 仅供网络安全研究、教学及故障排查使用。

*   **非商业用途**: 未经授权，禁止将本项目用于商业产品集成或销售。
*   **免责声明**: 使用本工具进行网络分析时请遵守当地法律法规，开发者不对使用本工具造成的任何后果负责。

License: **MIT (Non-Commercial Use Suggested)**
